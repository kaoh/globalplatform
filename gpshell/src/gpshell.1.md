...
:    -reader *readerName* Connect to card in the reader with *readerName*
    ...
    -readerNumber *x* Connect to card in the *x* th reader in the system

-open_sc

:    -keyind *x* -keyver *x* -key *key* -mac_key *mac-key* -enc_key *enc-key* -kek_key *kek-key* -security *securityLevel* -scp *protocol* -scpimpl *impl* -keyDerivation *derivation* Open secure channel
    ...
    If you have a card which uses key derivation you must enable the derivation mode with the -keyDerivation option and you must specify with -key the master (mother) key. -kek_key, -mac_key and -enc_key are not relevant is this case. See the section Options and Key derivation.
...
:   -file *appletFile* -priv *privilege* -sdAID *sdAID* -AID *AIDInPkg* -pkgAID *packageAID* -instAID *instanceAID* -nvCodeLimit *x* -nvDataLimit *y* Load and installs in one step
...
:    -pkgAID *packageAID* -sdAID *sdAID* -nvCodeLimit *y* Install for Load
...
It is useful to use some ASN.1 parser to interpret these data, like https://lapo.it/asn1js
Some useful identifier are:
.RS
9F7F - CPLC (Card Production Life Cycle) Data
.RE
.RS
00E0 - Key Information Templates. Instead of the first byte 00 also 01, ... can be used to get more key information templates if available.
There is a dedicated command for getting this: get_key_information_templates
.RE
.RS
